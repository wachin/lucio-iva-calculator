...
$distDir = Join-Path $workspaceRoot "build\dist-nuitka"
...
$portableZipPath = Join-Path $outputDir "LucioIVACalculator-$version-Windows-x64-nuitka-portable.zip"
...
$nsiContent = $nsiContent.Replace('!define INSTALLER_NAME "LucioIVACalculator-0.1.0-setup.exe"', "!define INSTALLER_NAME `"LucioIVACalculator-$version-nuitka-setup.exe`"")
...
if (Test-Path $makensis) {
    ...
    Move-Item "LucioIVACalculator-*-nuitka-setup.exe" "$outputDir" -Force
    Pop-Location
} else {
    Write-Warning "NSIS was not found. Nuitka portable ZIP was created, but the installer was not built."
}
